...
本机应用数据库为 `vip_erp_dev`，没有演示商品或假销量。测试使用自动新建的独立数据库，不混入应用数据。首次试用顺序：
...
无真实销售数据时，采购建议会明确跳过，仍可手工创建采购单。本机默认不配置真实 VOP；生产环境已启用只读商品目录同步，魔方罗盘取数仍等待平台提供公钥登记入口和指标 API code。
...
- 独立 VOP 接入边界、OAuth 与只读商品目录同步；魔方罗盘 RSA 认证及只读探针；BullMQ Worker 任务和失败重试验证。
