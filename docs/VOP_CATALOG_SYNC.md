...
`004_vop_details.sql` 增加独立详情、任务、状态及字典缓存表。Worker 按档期商品款号调用 `vipapis.product.VendorProductService.multiGetProductSkuInfo`（发布库 source=1，单页20条），空款号使用条码。接口不接受仅供应商ID的无条件分页，不能省略款号/条码。每批最多20页，请求间隔1秒，逐款保存分页，完成后1小时再次核对；手动同步会提前到期。首次为渐进补齐，列表显示已匹配条数及详情同步时间。品牌和品类名称来自 `BrandService.getBrandInfo` 与 `CategoryService.getCategoryById`，缓存7天。

按 namespace + barcode 保存并关联颜色（销售属性134）、尺码（453）、品牌、品类、图片、market_price/sell_price/supply_price及币种。该映射已用供应商服装样本验证；其他类目销售属性需另行核对。返回0和字段缺失分别保存，页面不将资料售价当作实时成交价。图片限定唯品会 vpimg CDN，通过HTTPS展示。详情失败保留上一版数据和页码，5分钟重试并显示错误。不会更新内部商品、SKU、库存或采购数据，因此内部单款手动编辑不受同步覆盖。

验证：`pnpm exec tsx tests/integration/vop-details.ts`（独立临时测试数据库）。回滚关闭 Worker 或恢复旧代码，保留新增表和已保存详情。
